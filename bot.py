...
FREE_CONSULTATION_TEXT = (
    ...
    "• Безопасно рассказать о самой деликатной проблеме в интимной сфере\n"
    ...
    "• Узнать, когда стоит обратиться к врачу (уролог, гинеколог, психотерапевт и пр.)\n"
    "• Задать уточняющие вопросы и почувствовать опору\n\n"
    "🧑‍⚕️ **Консультация сексолога длится 45 минут\n\n"
    "Консультация не заменяет приём живого специалиста, но даёт ясность и направление.\n\n"
    "🎁 Ваша первая консультация **бесплатная!**\n\n"
...
DEFAULT_WELCOME = (
    ...
    "Моя задача — не ставить диагнозов, а помочь разобраться в ситуации, предложить информацию и, если потребуется, дать рекомендации, основанные на проверенных методах. Вы можете рассказать о том, что вас беспокоит, — начиная с самых общих ощущений и заканчивая конкретными трудностями. Здесь нет тем, которые нельзя обсуждать, и нет оценок — только бережное отношение и профессиональная поддержка.\n\n"
...
async def generate_welcome_message() -> str:
    try:
        messages = [
            ...
            {"role": "user", "content": "Пользователь готов начать разговор. Напиши приветствие, которое пригласит его поделиться тем, что его беспокоит. Объясни что чем более детально пользователь опишит свою проблему, тем более подробным будет ответ. Упомени, что здесь можно не бояться обсудить самую деликатную проблему в интимной сфере. Сохрани свой обычный тон. Не используй Markdown, просто текст."}
        ]
        response = await asyncio.to_thread(
            openai.ChatCompletion.create,
            model="deepseek-chat",
